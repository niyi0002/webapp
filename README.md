...
Session based authentication for the user is realized with the help of Passport which is a middleware for node.js with a mongoDB backend. It uses cookies and sees the node application and database and has the node application and database working together to track the users session state. Another middleware used is express session to help to save session cookies. The Tvmaze API is used to fetch various data such as title, description, rating and images from Tvmaze Tvshow API.
